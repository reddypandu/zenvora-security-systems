...
import {
  ...
  Star, Clock, Users, Award, ShieldAlert,
} from 'lucide-react';
...
const whyUs = [
  { title: 'Certified Engineers',  desc: 'All technicians are industry-certified and background-verified.' },
  { title: 'End-to-End Service',   desc: 'Survey, supply, install, configure, and maintain — all under one roof.' },
  { title: '24/7 Support',         desc: 'Round-the-clock support with guaranteed 4-hour response time.' },
  { title: 'Transparent Pricing',  desc: 'No hidden costs. Detailed quotes before any work begins.' },
];
 ...
 *  - subtitle   : string  — short tagline / badge text (optional)
 *  - fullDesc   : string  — long description paragraph
 *  - features   : string[] — feature bullet points
 *  - image      : imported image or null
 *  - icon       : JSX element (large)
 *  - iconSm     : JSX element (small)
 *  - related    : [{ title, slug, image, icon, desc }]
 */
const ServicePageLayout = ({
  title, subtitle, fullDesc, features,
  image, icon, iconSm, related,
}) => {
  ...
  return (
    <div style={{ background: 'var(--bg-dark)', minHeight: '100vh', color: 'var(--text-main)', fontFamily: "'Outfit', sans-serif" }}>
      ...
      {/* ── HERO ── */}
      <div style={{ position: 'relative', paddingTop: '7rem', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '3rem', alignItems: 'center' }}>
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.75rem', background: 'rgba(194,0,4,0.1)', border: '1px solid rgba(194,0,4,0.25)', borderRadius: '9999px', padding: '0.4rem 1rem', marginBottom: '1.5rem' }}>
                <div style={{ color: 'var(--secondary)' }}>{iconSm}</div>
                <span style={{ color: 'var(--secondary)', fontWeight: '700', fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.2em' }}>
                  {subtitle || 'Zenvora Security'}
                </span>
              </div>
              ...
              <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', lineHeight: '1.8', maxWidth: '560px', marginBottom: '2.5rem' }}>
                {fullDesc}
              </p>
            ...
            >
              {image ? (
                <img src={image} alt={title} style={{ width: '100%', height: '380px', objectFit: 'cover', display: 'block' }} />
              ) : (
                <div style={{ width: '100%', height: '380px', background: 'linear-gradient(135deg, rgba(194,0,4,0.15), rgba(26,26,26,0.9))', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '1rem' }}>
                  <div style={{ color: 'var(--secondary)', opacity: 0.45, transform: 'scale(3)' }}>{iconSm}</div>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.85rem', marginTop: '3rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>{title}</p>
                </div>
              )}
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(2,6,23,0.75) 0%, transparent 60%)' }} />
        ...
        <div style={{ background: 'rgba(255,255,255,0.02)', borderTop: '1px solid rgba(255,255,255,0.05)', marginTop: '4rem' }}>
          <div className="container" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
            {stats.map((s, i) => (
              <div key={i} style={{ padding: '1.5rem 1rem', borderRight: i < 3 ? '1px solid rgba(255,255,255,0.05)' : 'none', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <div style={{ color: 'var(--secondary)' }}>{s.icon}</div>
                <div>
                  <p style={{ fontWeight: '800', fontSize: '1.25rem', lineHeight: 1 }}>{s.value}</p>
                  <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>{s.label}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1.25rem' }}>
            {features.map((feature, i) => (
              <motion.div
                ...
                style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '1rem', padding: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', cursor: 'default', transition: 'border-color 0.3s, background 0.3s' }}
      ...
      <div style={{ padding: '5rem 0', background: 'rgba(255,255,255,0.015)', borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
        <div className="container" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '3rem', alignItems: 'start' }}>
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            ...
            <p style={{ color: 'var(--text-muted)', lineHeight: '1.9', fontSize: '1.05rem', marginBottom: '1.25rem' }}>
              With over a decade of experience in {title}, our certified engineers deliver end-to-end support — from initial site survey to final installation and ongoing maintenance.
            </p>
            <p style={{ color: 'var(--text-muted)', lineHeight: '1.9', fontSize: '1.05rem' }}>
              We use only internationally-certified equipment, backed by our industry-leading AMC plans, delivering on time, every time.
            </p>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {whyUs.map((item, i) => (
        ...
        <div style={{ padding: '5rem 0' }}>
          <div className="container">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '2.5rem' }}>
              ...
              <Link
                to="/services"
                style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', color: 'var(--secondary)', fontWeight: '700', fontSize: '0.875rem', textDecoration: 'none', transition: 'gap 0.2s' }}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1.25rem' }}>
              {related.map((rel, i) => (
                <motion.div key={rel.slug} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                  <Link
      ...
      <div style={{ background: 'linear-gradient(135deg, rgba(194,0,4,0.2), rgba(194,0,4,0.05))', borderTop: '1px solid rgba(194,0,4,0.2)', padding: '4rem 0' }}>
        <div className="container" style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: '2rem' }}>
          <div>
            <h2 style={{ fontSize: 'clamp(1.5rem, 3vw, 2.25rem)', fontWeight: '800', marginBottom: '0.5rem' }}>Ready to secure your premises?</h2>
            <p style={{ color: 'var(--text-muted)', fontSize: '1rem' }}>Talk to our experts today — no obligation, no hidden fees.</p>
          </div>
          <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <Link
              to="/services"
              style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.85rem 2rem', fontSize: '1rem', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '0.5rem', color: 'var(--text-main)', textDecoration: 'none', fontWeight: '600', transition: 'background 0.2s' }}
